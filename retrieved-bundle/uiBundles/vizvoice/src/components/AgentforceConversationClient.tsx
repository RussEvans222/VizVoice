...
interface CustomElementRegistryWithDefine extends CustomElementRegistry {
	define(
		name: string,
...
function withLightningOutDefineGuard(run: () => void): void {
	const registry = customElements as CustomElementRegistryWithDefine;
	...
	registry.define = (
		...
		if (name === "lightning-out-application" && customElements.get(name)) {
			return;
		}
